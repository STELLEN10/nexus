...
export default function DMPage() {
  ...
  const otherUser = otherMsg ? { username: otherMsg.senderName, displayName: otherMsg.senderName, avatar: otherMsg.senderAvatar } : null;
  const online = useOnlineStatus(otherMsg?.senderId);

  useEffect(() => { bottomRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages.length]);

  // Focus input when keyboard appears — scroll to bottom
  useEffect(() => {
  ...
  return (
    <div className="chat-window">
      <div className="chat-topbar">
        ...
        {/* Tappable avatar in DM header */}
        <VibeAvatar
          user={otherUser}
          uid={otherMsg?.senderId}
          size={32}
          online={online}
          showVibe={true}
          tappable={true}
