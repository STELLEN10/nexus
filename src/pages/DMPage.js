  ...
  return (
    <div className="chat-window">
      <div className="chat-topbar">
        ...
        <div>
          <span className="dm-topbar-name">{otherName}</span>
      ...
      <div className="messages-area">
        ...
        {!loading && messages.length === 0 && (
          <div className="messages-empty"><div style={{ fontSize: 32 }}>💬</div><p>Start a conversation with {otherName}</p></div>
        )}
        {messages.map((msg, i) => {
          const prev = messages[i-1];
          const isOwn = msg.senderId === user?.uid;
          const isGrouped = prev && prev.senderId === msg.senderId && msg.createdAt - prev.createdAt < 300000;
          return <DMBubble key={msg.id} message={msg} isOwn={isOwn} isGrouped={isGrouped} onReply={() => setReplyTo(msg)} onReact={e => reactToDM(msg.id, e)} />;
        })}
        <div ref={bottomRef} />
      </div>
      ...
      {replyTo && (
        <div className="reply-bar">
          <div><span className="reply-bar-label">↩ Replying to {replyTo.senderName}</span><span className="reply-bar-preview">{replyTo.type === "sticker" ? "🎭 Sticker" : replyTo.content?.substring(0,80)}</span></div>
      ...
      <form className="input-bar" onSubmit={handleSend}>
        <div className="input-bar-inner">
          ...
          <textarea ref={inputRef} className="chat-textarea" placeholder={`Message ${otherName}`} value={input}
