...
export default function DMBubble({ message, isOwn, isGrouped, onReply, onReact }) {
  ...
  return (
    <div className={`dm-row ${isOwn ? "own" : ""} ${isGrouped ? "grouped" : ""}`} onMouseEnter={() => setHover(true)} onMouseLeave={() => { setHover(false); setShowPicker(false); }}>
      {!isOwn && (!isGrouped ? <Avatar user={{ username: message.senderName, avatar: message.senderAvatar }} size={28} /> : <div style={{ width: 28, flexShrink: 0 }} />)}
      <div className="dm-bubble-col">
        {!isGrouped && !isOwn && <div className="dm-meta"><span className="dm-name">{message.senderName}</span><span className="dm-ts">{fmt(message.createdAt)}</span></div>}
        {message.replyTo && <div className="dm-reply-ref"><span>{message.replyTo.senderName}</span><span>{message.replyTo.type === "sticker" ? "🎭 Sticker" : message.replyTo.content?.substring(0, 60)}</span></div>}
        <div className={`dm-bubble ${isOwn ? "own" : ""}`}>
          {isSticker ? <img src={message.content} alt="sticker" className="msg-sticker" /> : <span className="dm-txt">{message.content}</span>}
        </div>
        {isOwn && <div className="dm-receipt">{readByOther ? <span className="seen">✓✓ Seen</span> : <span className="sent">✓ Sent</span>}</div>}
