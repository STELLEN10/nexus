...
import {
  ...
  query, limit, getDoc, setDoc, addDoc,
  serverTimestamp
...
function UsersTab() {
  ...
  useEffect(() => {
    getDocs(query(collection(db, "users"), limit(100)))
      .then(snap => { setUsers(usersFromSnapshot(snap)); setLoading(false); })
      .catch(err => { setMsg("Error loading users: " + err.message); setLoading(false); });
  }, []);
      ...
      <div style={{ width: 260, flexShrink: 0, borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "12px 14px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
          <input
            ...
            style={{ width: "100%", padding: "8px 12px", background: "var(--bg-2)", border: "1.5px solid var(--border)", borderRadius: 10, color: "var(--text)", fontFamily: "var(--font)", fontSize: 12, outline: "none" }}
          />
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
        {!sel ? (
          ...
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20, padding: 16, background: "var(--bg-2)", borderRadius: 16, border: "1.5px solid var(--border)" }}>
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
        {/* User picker */}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 8 }}>1. Select User</div>
          <input
            placeholder="Search users..."
            value={search}
            onChange={e => setSearch(e.target.value)}
        ...
        {/* Badge picker */}
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 8 }}>2. Select Badge</div>
      ...
      {selUser && selBadge && (
        <div style={{
          ...
          <button onClick={handleAward} disabled={loading} style={{
            ...
            cursor: "pointer", flexShrink: 0, boxShadow: "0 0 14px var(--glow-purple)",
          }}>
...
function CoinsTab() {
  ...
  const handleGive = async () => {
    ...
    try {
      ...
      if (snap.exists()) {
        await updateDoc(coinRef, { balance: (snap.data().balance || 0) + amount });
      } else {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 560 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {TYPES.map(t => (
          <button key={t.id} onClick={() => setType(t.id)} style={{
            padding: "7px 14px",
            border: `1.5px solid ${type === t.id ? t.color + "66" : "var(--border)"}`,
      ...
      <button onClick={handleSend} disabled={loading || !title.trim() || !body.trim()}
        style={{ width: "100%", background: "linear-gradient(135deg,var(--accent),var(--accent-2))", border: "none", borderRadius: 12, padding: 13, color: "#fff", fontFamily: "var(--font)", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 0 16px var(--glow-purple)" }}>
