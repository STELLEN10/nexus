...
async function ensureGeneralRoom(uid) {
  ...
  if (!s.exists()) {
    await setDoc(r, { id: "general", name: "general", description: "Welcome to Nexus!", type: "public", createdBy: uid, createdAt: serverTimestamp(), lastActivity: serverTimestamp() });
  }
}
...
export function useMessages(roomId) {
  ...
  const sendMessage = useCallback(async (content, replyTo = null) => {
    ...
    await push(ref(rtdb, `messages/${roomId}`), { content: content.trim(), senderId: user.uid, senderName: profile?.displayName || profile?.username || "User", senderAvatar: profile?.avatar || "", replyTo: replyTo ? { id: replyTo.id, content: replyTo.content, senderName: replyTo.senderName } : null, createdAt: Date.now(), edited: false, reactions: {} });
...
export function useTyping(roomId) {
  ...
  useEffect(() => {
    ...
    const handleValue = (snap) => {
      const d = snap.val() || {};
      setTypingUsers(Object.entries(d).filter(([uid]) => uid !== user.uid).map(([, v]) => v.username));
    };
    onValue(typRef, handleValue);
    return () => {
      off(typRef, "value", handleValue);
  ...
  const startTyping = useCallback(() => {
    if (!isTypingRef.current) {
      ...
      set(ref(rtdb, `typing/${roomId}/${user.uid}`), { username: profile?.displayName || "User" });
    }
